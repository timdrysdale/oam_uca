...
offset_list = phi_list

switch_rel_period = relative_period(switch_frequency,sample_frequency)

for (phi, offset) in zip(phi_list, offset_list):      

    pos = vec3()
    pos.set_cylindrical(array_radius,phi,zero_z)
    array.append(AntennaSine(pos,switch_rel_period,offset))
...
ax.dist = 10
plt.savefig('sineUCA8-linearRx-3D.pdf', bbox_inches='tight', pad_inches=0)   
plt.show()
...
for rx in line_rx:
    ...
    for ant in array:
        ...
        if (use_time_switching == True):
            masked_signal = sine_mask(ant.rel_period,ant.offset, signal)
        else:
...
switch_peaks = detect_peaks(np.clip(example_amp_switch,-35,0))
...
plt.xlim([0.92,1.08])                                
plt.ylim([-40, 1])
plt.savefig('sineUCA8_switch_fft.pdf',bbox_inches='tight', pad_inches=0) 
plt.show()   
...
plt.plot(xlist,lamps0,'k-', label='0.98 GHz')
plt.plot(xlist,lamps1,'k-', label='1.02 GHz')
'''
...
plt.ylim([25,103])
plt.savefig('sineUCA8-linear-powerdB.pdf', bbox_inches='tight', pad_inches=0)  
plt.show()
...
ax.dist = 10
plt.savefig('sineUCA8-circularRx-3D.pdf', bbox_inches='tight', pad_inches=0)     
plt.show()
...
plt.plot(ang,cphases0 - 1 * p2, 'k-', label='0.98 GHz')
plt.plot(ang,cphases1 + 1 * p2, 'k-', label='1.02 GHz')
'''
...
plt.xlim([0,360])
plt.savefig('sineUCA8-circular-phase.pdf', bbox_inches='tight', pad_inches=0)  
plt.show()
...
plt.plot(ang,mode_purity0, 'k-', label='0.98 GHz')
plt.plot(ang,mode_purity1, 'k-' , label='1.02 GHz')
'''
...
plt.savefig('sineUCA8_purity.pdf', bbox_inches='tight', pad_inches=0) 
plt.show()
...
plt.savefig('sineUCA8signals.pdf', bbox_inches='tight', pad_inches=0) 
plt.show()